...
Create a new Supabase Edge Function for backend services in your KMP ERP.
...
**Scheduled Function** (with pg_cron - Daily reconciliation)
```typescript
serve(async () => {
  const supabase = createClient(url, serviceKey)
  // Run daily inventory cleanup
  await supabase.from('old_records').delete().lt('created_at', oldDate)
  return new Response('Done', { status: 200 })
})
```

**API Proxy/Transform** (Third-party integrations)
```typescript
serve(async (req) => {
...
**Business Logic Processing** (ERP operations)
```typescript
serve(async (req) => {
  ...
  return new Response(JSON.stringify({ success: true, shipment }), { status: 200 })
})
```
...
```kotlin
// In your KMP shared code or Android/iOS specific code
val response = httpClient.post("$supabaseUrl/functions/v1/function-name") {
  headers {
    append("Authorization", "Bearer $anonKey")
    append("Content-Type", "application/json")
  }
  setBody(json { put("key", "value") })
}
```
...
Generate production-ready Edge Functions with proper error handling, authentication, and type safety for your KMP ERP backend.
